...
const int THREADS_PER_BLOCK_NMS = sizeof(uint32_t) * 8; 
...
struct Point{
    ...
    __device__ Point operator +( Point &b) {
...
__device__ inline int check_in_box2d( float* box, const Point &p){
    //params:box (5) [x1,y1,x2,y2,angle]
     float MARGIN=1e-5;

    float center_x=(box[0]+box[1])/2;
...
__device__ inline void rotate_around_center( const Point &center,  float angle_cos,  float angle_sin,Point &p){
...
__device__ inline float box_overlap( float *box_a,  float *box_b){
...
__device__ inline float iou_bev( float *box_a,  float *box_b){
    // params: box_a (5) [x1, y1, x2, y2, angle]
    // params: box_b (5) [x1, y1, x2, y2, angle]
...
__global__ void boxes_overlap_kernel( int num_a,  float *boxes_a,  int num_b,  float *boxes_b, float *ans_overlap){
     int a_idx = blockIdx.y * THREADS_PER_BLOCK + threadIdx.y;
     int b_idx = blockIdx.x * THREADS_PER_BLOCK + threadIdx.x;
    
    if (a_idx >= num_a || b_idx >= num_b){
        return;
    }
     float * cur_box_a = boxes_a + a_idx * 5;
     float * cur_box_b = boxes_b + b_idx * 5;
    float s_overlap = box_overlap(cur_box_a, cur_box_b);
    ans_overlap[a_idx * num_b + b_idx] = s_overlap;
}

__global__ void boxes_iou_bev_kernel( int num_a,  float *boxes_a,  int num_b,  float *boxes_b, float *ans_iou){
     int a_idx = blockIdx.y * THREADS_PER_BLOCK + threadIdx.y;
     int b_idx = blockIdx.x * THREADS_PER_BLOCK + threadIdx.x;
...
__global__ void nms_kernel( int boxes_num,  float nms_overlap_thresh,
     float *boxes, uint32_t *mask){
//params: boxes (N, 5) [x1, y1, x2, y2, ry]
//params: mask (N, N/THREADS_PER_BLOCK_NMS)
 int row_start = blockIdx.y;
 int col_start = blockIdx.x;

 int row_size = fminf(boxes_num - row_start * THREADS_PER_BLOCK_NMS, THREADS_PER_BLOCK_NMS);
 int col_size = fminf(boxes_num - col_start * THREADS_PER_BLOCK_NMS, THREADS_PER_BLOCK_NMS);
...
if (threadIdx.x < row_size) {
 int cur_box_idx = THREADS_PER_BLOCK_NMS * row_start + threadIdx.x;
 float *cur_box = boxes + cur_box_idx * 5;
...
__device__ inline float iou_normal(float  *  a, float  *  b) {
...
__global__ void nms_normal_kernel( int boxes_num,  float nms_overlap_thresh,
     float *boxes, uint32_t *mask){
//params: boxes (N, 5) [x1, y1, x2, y2, ry]
//params: mask (N, N/THREADS_PER_BLOCK_NMS)
 int row_start = blockIdx.y;
...
void boxesoverlapLauncher( int num_a,  float *boxes_a, 
     int num_b,  float *boxes_b, float *ans_overlap){
...
void nmsLauncher( float *boxes, uint32_t * mask, 
...
void nmsNormalLauncher( float *boxes, uint32_t * mask, int boxes_num, 
