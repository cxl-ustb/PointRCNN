...
loc_bin_size, num_head_bin, anchor_size,
    ...
    if get_xz_fine:
        ...
        x_res_norm=paddle.squeeze(paddle.take_along_axis(pred_reg[:,x_res_l:x_res_r],indices=paddle.unsqueeze(x_bin,axis=1),axis=1),axis=1)
        # x_res_norm=paddle.squeeze(paddle_gather(pred_reg[:,x_res_l:x_res_r],dim=1,index=paddle.unsqueeze(x_bin,axis=1)),
        #                           axis=1)
        # 根据 z_bin的索引获得对应的z_res索引
        z_res_norm =paddle.squeeze(paddle.take_along_axis(pred_reg[:, z_res_l:z_res_r],indices=paddle.unsqueeze(z_bin, axis=1),axis=1),axis=1)
        # z_res_norm = paddle.squeeze(paddle_gather(pred_reg[:, z_res_l:z_res_r], dim=1,index=paddle.unsqueeze(z_bin, axis=1)),
        #                           axis=1)
    ...
    if get_y_by_bin:
        ...
        y_res_norm=paddle.squeeze(paddle.take_along_axis(pred_reg[:,y_res_l:y_res_r],axis=1,indices=paddle.unsqueeze(y_bin,axis=1)),axis=1)
    ...
    ry_res_norm =paddle.squeeze(paddle.take_along_axis(pred_reg[:,ry_res_l:ry_res_r],indices=paddle.unsqueeze(ry_bin,axis=1),axis=1),axis=1)
    # ry_res_norm=paddle.squeeze(paddle_gather(pred_reg[:,ry_res_l:ry_res_r],dim=1,index=paddle.unsqueeze(ry_bin,axis=1)),axis=1)
    ...
    if roi_box3d.shape[1] is 7:

        roi_ry=roi_box3d[:,6]
        ret_box3d=rotate_pc_along_paddle(shift_ret_box3d,-roi_ry)
        ret_box3d[:,6]+=roi_ry

    ret_box3d[:,0]+=roi_center[:,0]

    ret_box3d[:, 2] += roi_center[:,2]
