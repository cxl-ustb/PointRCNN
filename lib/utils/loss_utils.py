...
class DiceLoss(nn.Layer):
    ...
    def forward(self, input, target):
        ...
        input=F.sigmoid(paddle.reshape(input,[-1]))
        target=paddle.reshape(target.astype('float32'),[-1])
        mask = (target != self.ignore_target).astype('float32')


        return 1.0 - (paddle.minimum(input, target) * mask).sum() / paddle.clip((paddle.maximum(input, target) * mask).sum(),
                                                                           min=1.0)
...
def onehot(x_bin_label, per_loc_bin_num):
    onehot = np.zeros([x_bin_label.shape[0], per_loc_bin_num], dtype='int32')
    x_bin_label = x_bin_label.numpy()
    for i in range(x_bin_label.shape[0]):
        onehot[i, x_bin_label[i]] = 1
    return paddle.to_tensor(onehot)
...
def get_reg_loss(pred_reg, reg_label, loc_scope, loc_bin_size, num_head_bin, anchor_size,
    ...
    x_shift = paddle.clip(x_offset_label + loc_scope, 0, loc_scope * 2 - 1e-3)
    z_shift = paddle.clip(z_offset_label + loc_scope, 0, loc_scope * 2 - 1e-3)
    x_bin_label = paddle.floor(x_shift / loc_bin_size).astype('long')
    z_bin_label=paddle.floor(z_shift / loc_bin_size).astype('long')
    ...
    reg_loss_dict['loss_x_bin'] = loss_x_bin.item(0)
    reg_loss_dict['loss_z_bin'] = loss_z_bin.item(0)
    ...
    if get_xz_fine:
        ...
        x_res_label = x_shift - (x_bin_label.astype('float32') * loc_bin_size + loc_bin_size / 2)
        z_res_label = z_shift - (z_bin_label.astype('float32') * loc_bin_size + loc_bin_size / 2)
        x_res_norm_label = x_res_label / loc_bin_size
        z_res_norm_label = z_res_label / loc_bin_size



        # x_bin_onehot=paddle.zeros([x_bin_label.size(0), per_loc_bin_num],dtype='float32')
        # x_bin_onehot.scatter_(1, x_bin_label.view(-1, 1).long(), 1)
        # x_bin_onehot=onehot(x_bin_onehot,paddle.reshape(x_bin_label,[-1,1]))
        x_bin_onehot=onehot(x_bin_label,per_loc_bin_num)


        z_bin_onehot=onehot(z_bin_label,per_loc_bin_num)

        loss_x_res = F.smooth_l1_loss((pred_reg[:, x_res_l: x_res_r] * x_bin_onehot).sum(1), x_res_norm_label)
        loss_z_res = F.smooth_l1_loss((pred_reg[:, z_res_l: z_res_r] * z_bin_onehot).sum(1), z_res_norm_label)
        reg_loss_dict['loss_x_res'] = loss_x_res.item(0)
        reg_loss_dict['loss_z_res'] = loss_z_res.item(0)
    ...
    if get_y_by_bin:
        ...
        y_shift = paddle.clip(y_offset_label + loc_y_scope, 0, loc_y_scope * 2 - 1e-3)
        y_bin_label = (y_shift / loc_y_bin_size).floor().long()
        y_res_label = y_shift - (y_bin_label.astype('float32') * loc_y_bin_size + loc_y_bin_size / 2)
        y_res_norm_label = y_res_label / loc_y_bin_size


        y_bin_onehot=onehot(y_bin_label,loc_y_bin_num)

        loss_y_bin = F.cross_entropy(pred_reg[:, y_bin_l: y_bin_r], y_bin_label)
        loss_y_res = F.smooth_l1_loss((pred_reg[:, y_res_l: y_res_r] * y_bin_onehot).sum(1), y_res_norm_label)

        reg_loss_dict['loss_y_bin'] = loss_y_bin.item(0)
        reg_loss_dict['loss_y_res'] = loss_y_res.item(0)
        ...
        loss_y_offset = F.smooth_l1_loss(pred_reg[:, y_offset_l: y_offset_r].sum(1).astype('float32'), y_offset_label.astype('float32'))
        reg_loss_dict['loss_y_offset'] = loss_y_offset.item(0)
    ...
    if get_ry_fine:
        ...
        ry_bin_label = (shift_angle / angle_per_class).floor().astype('long')
        ry_res_label = shift_angle - (ry_bin_label.astype('float32') * angle_per_class + angle_per_class / 2)
    ...
    ry_bin_onehot=onehot(ry_bin_label,num_head_bin)
    loss_ry_bin = F.cross_entropy(pred_reg[:, ry_bin_l:ry_bin_r], ry_bin_label)
    loss_ry_res = F.smooth_l1_loss((pred_reg[:, ry_res_l: ry_res_r] * ry_bin_onehot).sum(1), ry_res_norm_label)

    reg_loss_dict['loss_ry_bin'] = loss_ry_bin.item(0)
    reg_loss_dict['loss_ry_res'] = loss_ry_res.item(0)
