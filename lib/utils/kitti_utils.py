...
def dist_to_plane(plane,points):
    '''
    Calculate the signed distance from a 3D plane to each point in a list of points
    :param plane: (a,b,c,d)
    :param points: (N,3)
...
def rotate_pc_along_y_paddle(pc, rot_angle):
    """
    :param pc: (N, 512, 3 + C)
    :param rot_angle: (N)
    :return:
    ...
    R=paddle.concat([raw_1.unsqueeze(1),raw_2.unsqueeze(1)],axis=1)
    # (N, 2, 2)
    ...
    pc_temp=paddle.concat([pc_temp0.unsqueeze(-1),pc_temp2.unsqueeze(-1)],axis=-1)

    pc_temp=  paddle.matmul(pc_temp, paddle.transpose(R,[0,2,1]))  # (N, 512, 1)
...
def boxes3d_to_bev_paddle(boxes3d):
    '''
    :param boxes3d: (N,7) [x,y,z,h,w,l,ry]
    :return:
           boxes_bev:(N,5) [x1,y1,x2,y2,ry]
    '''
    boxes_bev=paddle.ones([boxes3d.shape[0],5],dtype=boxes3d.dtype)
