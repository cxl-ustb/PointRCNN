    ...
    for data in dataloader:
        ...
        if not args.test:
            ...
            if gt_boxes3d.shape[1] == 0:  # (B, M, 7)
                ...
                gt_boxes3d =paddle.to_tensor(gt_boxes3d).astype('long')

        inputs = paddle.to_tensor(pts_input).astype('long')
        ...
        rpn_scores = paddle.nn.functional.sigmoid(rpn_scores_raw)
        seg_result = (rpn_scores > cfg.RPN.SCORE_THRESH).astype('long')
        ...
        for bs_idx in range(batch_size):
            ...
            if not args.test:
                ...
                if cur_gt_boxes3d.shape[0] > 0:
                    ...
                    gt_max_iou, _ = paddle.max(iou3d,axis=1),paddle.argmax(iou3d,axis=1)
                ...
                correct = ((cur_seg_result == cur_rpn_cls_label) & fg_mask).sum().astype('float32')
                union = fg_mask.sum().astype('float32') + (cur_seg_result > 0).sum().astype('float32') - correct
                rpn_iou = correct / paddle.clip(union, min=1.0)
                rpn_iou_avg += rpn_iou.item()

            # save result
            if args.save_rpn_feature:
                # save features to file
                save_rpn_features(seg_result[bs_idx].astype('float32').cpu().numpy(),
                                  rpn_scores_raw[bs_idx].astype('float32').cpu().numpy(),
                                  pts_features[bs_idx],
...
def eval_one_epoch_rcnn(model, dataloader, epoch_id, result_dir, logger):
    ...
    for data in dataloader:
        ...
        if cfg.RCNN.ROI_SAMPLE_JIT:
            for key, val in input_data.items():
                if key in ['gt_iou', 'gt_boxes3d']:
                    continue
                input_data[key] = input_data[key].unsqueeze(axis=0)
        else:
            pts_input = paddle.concat((input_data['pts_input'], input_data['pts_features']), axis=-1)
        ...
        if rcnn_cls.shape[1] == 1:
            ...
            norm_scores = paddle.nn.functional.sigmoid(raw_scores)
            pred_classes = (norm_scores > cfg.RCNN.SCORE_THRESH).astype('long')
        else:
            pred_classes = paddle.reshape(paddle.argmax(rcnn_cls, 1),[-1])
            cls_norm_scores = F.softmax(rcnn_cls, 1)
        ...
        if not args.test:
            ...
            if gt_num > 0:
                ...
                gt_max_iou= iou3d.max(0)
                refined_iou= iou3d.max(1)
            ...
            cls_label = (gt_iou > cfg.RCNN.CLS_FG_THRESH).astype('float32')
            cls_valid_mask = ((gt_iou >= cfg.RCNN.CLS_FG_THRESH) | (gt_iou <= cfg.RCNN.CLS_BG_THRESH)).astype('float32')
            cls_acc = ((pred_classes == cls_label.astype('long')).astype('float32') * cls_valid_mask).sum() / max(cls_valid_mask.sum(), 1.0)

            iou_thresh = 0.7 if cfg.CLASSES == 'Car' else 0.5
            cls_label_refined = (gt_iou >= iou_thresh).astype('float32')
            cls_acc_refined = (pred_classes == cls_label_refined.astype('long')).astype('float32').sum() / max(cls_label_refined.shape[0], 1.0)
        ...
        # NMS thresh
        boxes_bev_selected = kitti_utils.boxes3d_to_bev_paddle(pred_boxes3d_selected)
...
def eval_one_epoch_joint(model, dataloader, epoch_id, result_dir, logger):
    ...
    for data in dataloader:
        ...
        seg_result = ret_dict['seg_result'].astype('long')  # (B, N)

        rcnn_cls = ret_dict['rcnn_cls'].reshape([batch_size, -1, ret_dict['rcnn_cls'].shape[1]])
        rcnn_reg = ret_dict['rcnn_reg'].reshape([batch_size, -1, ret_dict['rcnn_reg'].shape[1]])  # (B, M, C)
        ...
        pred_boxes3d = decode_bbox_target(roi_boxes3d.reshape([-1, 7]), rcnn_reg.reshape([-1, rcnn_reg.shape[-1]]),
                                          anchor_size=anchor_size,
                                          loc_scope=cfg.RCNN.LOC_SCOPE,
                                          loc_bin_size=cfg.RCNN.LOC_BIN_SIZE,
                                          num_head_bin=cfg.RCNN.NUM_HEAD_BIN,
                                          get_xz_fine=True, get_y_by_bin=cfg.RCNN.LOC_Y_BY_BIN,
                                          loc_y_scope=cfg.RCNN.LOC_Y_SCOPE, loc_y_bin_size=cfg.RCNN.LOC_Y_BIN_SIZE,
                                          get_ry_fine=True).reshape([batch_size, -1, 7])
        ...
        if rcnn_cls.shape[2] == 1:
            ...
            pred_classes = paddle.argmax(rcnn_cls, axis=1).reshape(-1)
        ...
        if not args.test:
            ...
            for k in range(batch_size):
                ...
                if tmp_idx >= 0:
                    ...
                    cur_gt_boxes3d = paddle.to_tensor(cur_gt_boxes3d).astype('float32')
                ...
                if not cfg.RPN.FIXED:
                    ...
                    correct = ((seg_result == rpn_cls_label) & fg_mask).sum().astype('float32')
                    union = fg_mask.sum().astype('float32') + (seg_result > 0).sum().astype('float32') - correct
                    rpn_iou = correct / paddle.clip(union, min=1.0)
                    total_rpn_iou += rpn_iou.item(0)
        ...
        for k in range(batch_size):
            ...
            # rotated nms
            boxes_bev_selected = kitti_utils.boxes3d_to_bev_paddle(pred_boxes3d_selected)
            keep_idx = iou3d_utils.nms_gpu(boxes_bev_selected, raw_scores_selected, cfg.RCNN.NMS_THRESH).reshape(-1)
            pred_boxes3d_selected = pred_boxes3d_selected[keep_idx]
            scores_selected = raw_scores_selected[keep_idx]
            pred_boxes3d_selected, scores_selected = pred_boxes3d_selected.cpu().numpy(), scores_selected.cpu().numpy()
...
def eval_single_ckpt(root_result_dir):
    ...
    # load checkpoint
    model_state_dict=paddle.load('PointRCNN.pdparams')
