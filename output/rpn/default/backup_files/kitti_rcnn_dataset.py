...
class KittiRCNNDataset(KittiDataset):
    ...
    def get_rpn_features(rpn_feature_dir, idx):
        ...
        if cfg.RCNN.USE_SEG_SCORE:
            ...
            rpn_seg_score = paddle.nn.functional.sigmoid(paddle.to_tensor(rpn_seg_score)).numpy()
        else:
    ...
    def apply_gt_aug_to_one_scene(self, sample_id, pts_rect, pts_intensity, all_gt_boxes3d):
        ...
        while try_times > 0:
            ...
            boxes_pts_mask_list = roipool3d_utils.pts_in_boxes3d_cpu(
                paddle.to_tensor(pts_rect), paddle.to_tensor(enlarged_box3d.reshape(1, 7)))
    ...
    def get_rcnn_sample_info(self, roi_info):
        ...
        boxes_pts_mask_list = roipool3d_utils.pts_in_boxes3d_cpu(paddle.to_tensor(rpn_xyz),
                                                                 paddle.to_tensor(pooled_boxes3d))
        pt_mask_flag = (boxes_pts_mask_list[0].numpy() == 1)
        ...
        if aug_pts.shape[0] == 0:
            ...
            if cfg.RCNN.USE_MASK:
                if cfg.RCNN.MASK_TYPE == 'seg':
                    ...
                    pts_mask = roipool3d_utils.pts_in_boxes3d_cpu(paddle.to_tensor(aug_pts),
                                                                  paddle.reshape(paddle.to_tensor(aug_roi_box3d),[1,7])
                                                                  )
    ...
    def canonical_transform_batch(pts_input, roi_boxes3d, gt_boxes3d):
        ...
        # rotate to the direction of head
        gt_boxes3d_ct = kitti_utils.rotate_pc_along_y_paddle(

                                                            paddle.to_tensor(paddle.reshape(gt_boxes3d_ct,[-1,1,7])),
                                                            paddle.reshape(paddle.to_tensor(roi_ry)).numpy(),[-1,7]
        )
        gt_boxes3d_ct[:, 6] = gt_boxes3d_ct[:, 6] - roi_ry
        pts_input = kitti_utils.rotate_pc_along_y_paddle(paddle.to_tensor(pts_input), paddle.to_tensor(roi_ry)).numpy()
    ...
    def get_rcnn_training_sample_batch(self, index):
        ...
        if fg_num_rois > 0 and bg_num_rois > 0:
            ...
            rand_num = paddle.to_tensor(rand_num).astype('long') #.type_as(gt_boxes3d).long()
