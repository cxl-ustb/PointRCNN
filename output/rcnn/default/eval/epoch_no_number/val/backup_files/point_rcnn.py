...
class PointRCNN(nn.Layer):
    ...
    def forward(self, input_data):
        if cfg.RPN.ENABLED:
            output = {}
            # rpn inference
            with paddle.set_grad_enabled((not cfg.RPN.FIXED) and self.training):
                if cfg.RPN.FIXED:
                    self.rpn.eval()
            ...
            if cfg.RCNN.ENABLED:
                with paddle.no_grad():
                    ...
                    rpn_scores_norm = nn.functional.sigmoid(rpn_scores_raw)
                    seg_mask = (rpn_scores_norm > cfg.RPN.SCORE_THRESH).astype('float32')
                    pts_depth = paddle.norm(backbone_xyz, p=2,axis=2)
                ...
                rcnn_input_info = {'rpn_xyz': backbone_xyz,
                                   'rpn_features': paddle.transpose(backbone_features,[0,2,1]),
                                   'seg_mask': seg_mask,
                                   'roi_boxes3d': rois,
                                   'pts_depth': pts_depth}
                if self.training:
