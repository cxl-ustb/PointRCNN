...
from lib.utils.pointnet2.pointnet2_modules import PointnetSAModule
...
class RCNNNet(nn.Layer):
    def __init__(self, num_classes, input_channels=0, use_xyz=True):
        ...
        if cfg.RCNN.USE_RPN_FEATURES:
            ...
            self.xyz_up_layer = pd_utils.SharedMLP([self.rcnn_input_channel] + cfg.RCNN.XYZ_UP_LAYER,
                                                   bn=cfg.RCNN.USE_BN)
            c_out = cfg.RCNN.XYZ_UP_LAYER[-1]
            self.merge_down_layer = pd_utils.SharedMLP([c_out * 2, c_out], bn=cfg.RCNN.USE_BN)
        ...
        for k in range(0, cfg.RCNN.CLS_FC.__len__()):
            cls_layers.append(pd_utils.Conv1d(pre_channel, cfg.RCNN.CLS_FC[k], bn=cfg.RCNN.USE_BN))
            pre_channel = cfg.RCNN.CLS_FC[k]
        cls_layers.append(pd_utils.Conv1d(pre_channel, cls_channel, activation=None))
        ...
        if cfg.RCNN.LOSS_CLS == 'SigmoidFocalLoss':
            ...
            cls_weight = paddle.to_tensor(cfg.RCNN.CLS_WEIGHT)
        ...
        for k in range(0, cfg.RCNN.REG_FC.__len__()):
            reg_layers.append(pd_utils.Conv1d(pre_channel, cfg.RCNN.REG_FC[k], bn=cfg.RCNN.USE_BN))
    ...
    def forward(self, input_data):
        ...
        if cfg.RCNN.ROI_SAMPLE_JIT:
            if self.training:
                ...
                pts_input = paddle.concat((target_dict['sampled_pts'], target_dict['pts_feature']), 2)
                ...
                pooled_features[:, :, :, 0:3] -= paddle.unsqueeze(roi_center,2)
                for k in range(batch_size):
                    pooled_features[k, :, :, 0:3] = kitti_utils.rotate_pc_along_y_paddle(pooled_features[k, :, :, 0:3],
                                                                                        batch_rois[k, :, 6])


                pts_input =paddle.reshape(pooled_features,[-1, pooled_features.shape[2], pooled_features.shape[3]])
        else:
        ...
        if cfg.RCNN.USE_RPN_FEATURES:
            xyz_input=paddle.unsqueeze(paddle.transpose(pts_input[..., 0:self.rcnn_input_channel],[0,2,1]),3)
            xyz_feature = self.xyz_up_layer(xyz_input)

            rpn_feature =paddle.unsqueeze(paddle.transpose(pts_input[..., self.rcnn_input_channel:],[0,2,1]),3)
            merged_feature = paddle.concat((xyz_feature, rpn_feature), 1)
            merged_feature = self.merge_down_layer(merged_feature)

            l_xyz, l_features = [xyz], [paddle.squeeze(merged_feature,3)]
        else:
        ...
        rcnn_cls =paddle.squeeze(paddle.transpose(self.cls_layer(l_features[-1]),[0,2,1]),1) # (B, 1 or 2)
        rcnn_reg=paddle.squeeze(paddle.transpose(self.reg_layer(l_features[-1]),[0,2,1]),1)  # (B, C)
